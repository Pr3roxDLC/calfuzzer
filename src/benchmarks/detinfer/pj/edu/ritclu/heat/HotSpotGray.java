//******************************************************************************
//
// File:    HotSpotGray.java
// Package: benchmarks.detinfer.pj.edu.ritclu.heat
// Unit:    Class benchmarks.detinfer.pj.edu.ritclu.heat.HotSpotGray
//
...
import benchmarks.detinfer.pj.edu.ritimage.GrayImageRow;
...
/**
 * Class HotSpotGray is a sequential program that calculates the temperature
 * distribution over a metal plate with hot spots.
 * <P>
 * Usage: java benchmarks.detinfer.pj.edu.ritclu.heat.HotSpotGray <I>imagefile</I> <I>H</I> <I>W</I>
 ...
 * updating. The program outputs a PJG grayscale image <I>H</I>+2 pixels high
 * and <I>W</I>+2 pixels wide. Each pixel's gray level depends on the
 * corresponding mesh point's temperature. A temperature of 0.0 is black; a
 * temperature of 100.0 is white; intermediate temperatures are intermediate
 * shades of gray. The PJG image file is named <I>imagefile</I>.
 * <P>
...
// Hidden constants.
	...
	private static final double DELTA_GRAY = MAX_GRAY - MIN_GRAY;
...
// Main program.
	...
	public static void main
		...
		// Generate image.
		byte[][] matrix = new byte [H+2] [W+2];
		GrayImageRow matrix_r = new GrayImageRow (matrix[0]);
		for (int r = 0; r <= H+1; ++ r)
			{
			double[] h_r = h[r];
			...
			for (int c = 0; c <= W+1; ++ c)
				{
				matrix_r.setPixel (c, (float)
					((h_r[c]-MIN_TEMP)/DELTA_TEMP*DELTA_GRAY+MIN_GRAY));
				}
			}
		PJGGrayImage image = new PJGGrayImage (H+2, W+2, matrix);
...
// Hidden operations.
	...
	private static void usage()
		{
		System.err.println ("Usage: java benchmarks.detinfer.pj.edu.ritclu.heat.HotSpotGray <imagefile> <H> <W> <rl1> <cl1> <ru1> <cu1> <temp1> [ <rl2> <cl2> <ru2> <cu2> <temp2> . . . ]");
