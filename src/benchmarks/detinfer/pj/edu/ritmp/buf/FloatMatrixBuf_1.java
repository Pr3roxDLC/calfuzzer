//******************************************************************************
//
// File:    FloatMatrixBuf_1.java
// Package: benchmarks.detinfer.pj.edu.ritmp.buf
// Unit:    Class benchmarks.detinfer.pj.edu.ritmp.buf.FloatMatrixBuf_1
//
...
/**
 * Class FloatMatrixBuf_1 provides a buffer for a matrix of float items
 * sent or received using the Message Protocol (MP). The matrix row and column
 * strides must both be 1. While an instance of class FloatMatrixBuf_1 may
 * be constructed directly, normally you will use a factory method in class
 * {@linkplain benchmarks.detinfer.pj.edu.ritmp.FloatBuf FloatBuf}. See that class for further
 * information.
 *
...
// Exported constructors.
	...
	/**
	 * Construct a new float matrix buffer. It is assumed that the rows and
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new FloatMatrixReductionBuf_1
			(myMatrix, myRowRange, myColRange, (FloatOp) op);
		}
...
// Hidden operations.
	...
	protected int sendItems
		...
		while (r < myRowCount && ncols > 0)
			{
			floatbuffer.put (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			{
			floatbuffer.get (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
