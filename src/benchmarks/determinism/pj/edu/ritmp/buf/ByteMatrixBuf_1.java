//******************************************************************************
//
// File:    ByteMatrixBuf_1.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.ByteMatrixBuf_1
//
 ...
 * must both be 1. While an instance of class ByteMatrixBuf_1 may be constructed
 * directly, normally you will use a factory method in class {@linkplain
 * benchmarks.determinism.pj.edu.ritmp.ByteBuf ByteBuf}. See that class for further information.
 *
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new ByteMatrixReductionBuf_1
			(myMatrix, myRowRange, myColRange, (ByteOp) op);
		}
...
// Hidden operations.
	...
	protected int sendItems
		...
		while (r < myRowCount && ncols > 0)
			{
			buffer.put (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			{
			buffer.get (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
