//******************************************************************************
//
// File:    IntegerMatrixBuf_1.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.IntegerMatrixBuf_1
//
...
/**
 * Class IntegerMatrixBuf_1 provides a buffer for a matrix of integer items
 * sent or received using the Message Protocol (MP). The matrix row and column
 * strides must both be 1. While an instance of class IntegerMatrixBuf_1 may
 * be constructed directly, normally you will use a factory method in class
 * {@linkplain benchmarks.determinism.pj.edu.ritmp.IntegerBuf IntegerBuf}. See that class for further
 * information.
 *
 * @author  Alan Kaminsky
 * @version 03-Mar-2008
 */
public class IntegerMatrixBuf_1
	extends IntegerMatrixBuf
	{
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new IntegerMatrixReductionBuf_1
...
// Hidden operations.
	...
	protected int sendItems
		...
		while (r < myRowCount && ncols > 0)
			{
			intbuffer.put (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			{
			intbuffer.get (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
