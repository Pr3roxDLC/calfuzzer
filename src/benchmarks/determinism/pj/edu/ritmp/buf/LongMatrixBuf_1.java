//******************************************************************************
//
// File:    LongMatrixBuf_1.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.LongMatrixBuf_1
//
 ...
 * strides must both be 1. While an instance of class LongMatrixBuf_1 may
 * be constructed directly, normally you will use a factory method in class
 * {@linkplain benchmarks.determinism.pj.edu.ritmp.LongBuf LongBuf}. See that class for further
 * information.
 *
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new LongMatrixReductionBuf_1
...
// Hidden operations.
	...
	protected int sendItems
		...
		while (r < myRowCount && ncols > 0)
			{
			longbuffer.put (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			{
			longbuffer.get (myMatrix[row], col, ncols);
			n += ncols;
			++ r;
			++ row;
			c = 0;
