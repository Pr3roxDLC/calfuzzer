//******************************************************************************
//
// File:    CharacterArrayBuf_1.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.CharacterArrayBuf_1
//
 ...
 * must be 1. While an instance of class CharacterArrayBuf may be constructed
 * directly, normally you will use a factory method in class {@linkplain
 * benchmarks.determinism.pj.edu.ritmp.CharacterBuf CharacterBuf}. See that class for further
 * information.
 *
 * @author  Alan Kaminsky
 * @version 25-Oct-2007
 */
public class CharacterArrayBuf_1
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new CharacterArrayReductionBuf_1
			(myArray, myRange, (CharacterOp) op);
		}
...
// Hidden operations.
	...
	protected int sendItems
		...
		int n = Math.min (myLength - i, charbuffer.remaining());
		charbuffer.put (myArray, myArrayOffset + i, n);
	...
	protected int receiveItems
		...
		{
		CharBuffer charbuffer = buffer.asCharBuffer();
		int n = Math.min (num, Math.min (myLength - i, charbuffer.remaining()));
		charbuffer.get (myArray, myArrayOffset + i, n);
		buffer.position (buffer.position() + 2*n);
		return n;
		}
