//******************************************************************************
//
// File:    Signed16BitIntegerBuf.java
// Package: benchmarks.determinism.pj.edu.ritmp
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.Signed16BitIntegerBuf
//
...
import benchmarks.determinism.pj.edu.ritmp.buf.EmptySigned16BitIntegerBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.Signed16BitIntegerArrayBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.Signed16BitIntegerArrayBuf_1;
import benchmarks.determinism.pj.edu.ritmp.buf.Signed16BitIntegerItemBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.Signed16BitIntegerMatrixBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.Signed16BitIntegerMatrixBuf_1;
import benchmarks.determinism.pj.edu.ritmp.buf.SharedSigned16BitIntegerBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.SharedSigned16BitIntegerArrayBuf;
import benchmarks.determinism.pj.edu.ritmp.buf.SharedSigned16BitIntegerArrayBuf_1;

import benchmarks.determinism.pj.edu.ritpj.reduction.SharedInteger;
import benchmarks.determinism.pj.edu.ritpj.reduction.SharedIntegerArray;
 ...
 * 16-bit integer items sent or received using the Message Protocol (MP). In a
 * message, a signed 16-bit integer item is represented as two bytes, most
 * significant byte first. The range of values that can be represented is -32768
 * .. 32767.
 * <P>
 ...
 * <P>
 * To create a Signed16BitIntegerBuf, call one of the following static factory
 * methods:
 * <UL>
 ...
 * @version 03-May-2008
 */
public abstract class Signed16BitIntegerBuf
	extends Buf
	{
...
// Hidden constructors.
	 ...
	 */
	protected Signed16BitIntegerBuf
		(int theLength)
		{
...
// Exported operations.
	 ...
	 * is signed 16-bit integer.
	 *
	 * @return  Empty buffer.
	 */
	public static Signed16BitIntegerBuf emptyBuffer()
		{
		return new EmptySigned16BitIntegerBuf();
		}
	 ...
	 * @return  Buffer.
	 */
	public static Signed16BitIntegerItemBuf buffer()
		{
		return new Signed16BitIntegerItemBuf();
		}
	...
	public static Signed16BitIntegerItemBuf buffer
		(int item)
		{
		return new Signed16BitIntegerItemBuf (item);
		}
	 ...
	 */
	public static Signed16BitIntegerBuf buffer
		(int[] theArray)
		{
		if (theArray == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.buffer(): theArray is null");
			}
		int nr = Arrays.length (theArray);
		return new Signed16BitIntegerArrayBuf_1 (theArray, new Range (0, nr-1));
		}
	 ...
	 */
	public static Signed16BitIntegerBuf sliceBuffer
		(int[] theArray,
		 Range theRange)
		{
		if (theArray == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.sliceBuffer(): theArray is null");
			}
		int nr = Arrays.length (theArray);
		if (0 > theRange.lb() || theRange.ub() >= nr)
			{
			throw new IndexOutOfBoundsException
				("Signed16BitIntegerBuf.sliceBuffer(): theArray index range = 0.." +
				 (nr-1) + ", theRange = " + theRange);
			}
		if (theRange.stride() == 1)
			{
			return new Signed16BitIntegerArrayBuf_1 (theArray, theRange);
			}
		else
			{
			return new Signed16BitIntegerArrayBuf (theArray, theRange);
			}
		}
	 ...
	 */
	public static Signed16BitIntegerBuf[] sliceBuffers
		(int[] theArray,
		 Range[] theRanges)
		{
		int n = theRanges.length;
		Signed16BitIntegerBuf[] result = new Signed16BitIntegerBuf [n];
		for (int i = 0; i < n; ++ i)
			{
	...
	public static Signed16BitIntegerBuf buffer
		...
		if (theMatrix == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.buffer(): theMatrix is null");
			}
		int nr = Arrays.rowLength (theMatrix);
		int nc = Arrays.colLength (theMatrix, 0);
		return new Signed16BitIntegerMatrixBuf_1
	 ...
	 */
	public static Signed16BitIntegerBuf rowSliceBuffer
		(int[][] theMatrix,
		 Range theRowRange)
		{
		if (theMatrix == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.rowSliceBuffer(): theMatrix is null");
			}
		int nr = Arrays.rowLength (theMatrix);
		if (0 > theRowRange.lb() || theRowRange.ub() >= nr)
			{
			throw new IndexOutOfBoundsException
				("Signed16BitIntegerBuf.rowSliceBuffer(): theMatrix row index range = 0.." +
		...
		if (theRowRange.stride() == 1)
			...
			{
			return new Signed16BitIntegerMatrixBuf
				(theMatrix, theRowRange, new Range (0, nc-1));
			}
		}
	 ...
	 */
	public static Signed16BitIntegerBuf[] rowSliceBuffers
		(int[][] theMatrix,
		 Range[] theRowRanges)
		{
	 ...
	 */
	public static Signed16BitIntegerBuf colSliceBuffer
		(int[][] theMatrix,
		 Range theColRange)
		{
		if (theMatrix == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.colSliceBuffer(): theMatrix is null");
			}
		int nr = Arrays.rowLength (theMatrix);
		int nc = Arrays.colLength (theMatrix, 0);
		if (0 > theColRange.lb() || theColRange.ub() >= nc)
			{
			throw new IndexOutOfBoundsException
				("Signed16BitIntegerBuf.colSliceBuffer(): theMatrix column index range = 0.."
	 ...
	 */
	public static Signed16BitIntegerBuf[] colSliceBuffers
		(int[][] theMatrix,
		 Range[] theColRanges)
		{
	 ...
	 */
	public static Signed16BitIntegerBuf patchBuffer
		(int[][] theMatrix,
		 Range theRowRange,
		 Range theColRange)
		{
		if (theMatrix == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.patchBuffer(): theMatrix is null");
			}
		int nr = Arrays.rowLength (theMatrix);
		if (0 > theRowRange.lb() || theRowRange.ub() >= nr)
			{
			throw new IndexOutOfBoundsException
				("Signed16BitIntegerBuf.patchBuffer(): theMatrix row index range = 0.."
				 + (nr-1) + ", theRowRange = " + theRowRange);
			}
		int nc = Arrays.colLength (theMatrix, theRowRange.lb());
		if (0 > theColRange.lb() || theColRange.ub() >= nc)
			{
			throw new IndexOutOfBoundsException
				("Signed16BitIntegerBuf.patchBuffer(): theMatrix column index range = 0.."
				 + (nc-1) + ", theColRange = " + theColRange);
			}
		if (theRowRange.stride() == 1 && theColRange.stride() == 1)
			{
			return new Signed16BitIntegerMatrixBuf_1 (theMatrix, theRowRange, theColRange);
			}
		else
			{
			return new Signed16BitIntegerMatrixBuf (theMatrix, theRowRange, theColRange);
			}
		}
	 ...
	 *     <TT>theColRanges</TT>.
	 */
	public static Signed16BitIntegerBuf[] patchBuffers
		(int[][] theMatrix,
		 Range[] theRowRanges,
		 Range[] theColRanges)
		{
	 ...
	 * instance of class {@linkplain benchmarks.determinism.pj.edu.ritpj.reduction.SharedInteger
	...
	public static Signed16BitIntegerBuf buffer
		...
		if (item == null)
			{
			throw new NullPointerException
				("Signed16BitIntegerBuf.buffer(): item is null");
			}
		return new SharedSigned16BitIntegerBuf (item);
		}
	...
	public static Signed16BitIntegerBuf buffer
		...
		return new SharedSigned16BitIntegerArrayBuf_1 (theArray, new Range (0, nr-1));
		}
	...
	public static Signed16BitIntegerBuf sliceBuffer
		...
		if (theRange.stride() == 1)
			{
			return new SharedSigned16BitIntegerArrayBuf_1 (theArray, theRange);
			}
		else
			{
			return new SharedSigned16BitIntegerArrayBuf (theArray, theRange);
			}
		}
	...
	public void copy
		(Buf theSrc)
		{
		if (theSrc != this) defaultCopy ((Signed16BitIntegerBuf) theSrc, this);
		}
...
// Hidden operations.
	...
	protected static void defaultCopy
		(Signed16BitIntegerBuf theSrc,
		 Signed16BitIntegerBuf theDst)
		{
