//*****************************************************************************
//
// File:    MaximumParsimonyBnbHyb.java
// Package: benchmarks.determinism.pj.edu.ritcompbio.phyl
// Unit:    Class benchmarks.determinism.pj.edu.ritcompbio.phyl.MaximumParsimonyBnbHyb
//
...
/**
 * Class MaximumParsimonyBnbHyb provides a hybrid parallel algorithm for maximum
 * parsimony phylogenetic tree construction using branch-and-bound search.
 * <P>
 * Class MaximumParsimonyBnbHyb is designed to be used in a hybrid parallel
 * program that runs on multiple nodes with one process per node and multiple
 * threads per process. Each thread has its own MaximumParsimonyBnbHyb instance.
 * Each thread uses its own MaximumParsimonyBnbHyb instance to search different
 * sections of the search graph concurrently.
 * <P>
 * Each process has a shared, replicated variable, <I>bound</I>, that holds the
 * best parsimony score found so far (i.e., the bound for branch-and-bound
 * search). The <I>bound</I> variable is an instance of class {@linkplain
 * benchmarks.determinism.pj.edu.ritpj.replica.ReplicatedInteger}. All the MaximumParsimonyBnbHyb
 * instances within the process share the same <I>bound</I> variable. Whenever
 * one thread finds a phylogenetic tree with a better parsimony score, it
 * notifies all the threads and processes by updating the <I>bound</I> variable.
 * <P>
 * To perform a search, each process must:
 * <OL TYPE=1>
 * <LI>
 * Call the static <TT>createBoundVariable()</TT> method to create the shared,
 * replicated <I>bound</I> variable.
 * </OL>
 * Then each thread in each process must:
 * <OL TYPE=1 START=2>
 * <LI>
 * Create its own instance of class MaximumParsimonyBnbHyb, passing in a
 * {@linkplain DnaSequenceList} of the DNA sequences in the tree, a reference to
 * the process's shared, replicated <I>bound</I> variable, and a {@linkplain
 ...
 * </OL>
 * <P>
 * <I>Note:</I> Class MaximumParsimonyBnbHyb is not multiple thread safe; it is
...
// Exported static operations.
	...
	/**
	 * Create a new shared, replicated <I>bound</I> variable. The <I>bound</I>
	 * variable will use the world communicator and a message tag of
	 * <TT>Integer.MAX_VALUE</TT>. This message tag must not be used in any
	 * other messages in the program.
	 *
	 * @param  initialBound    Initial bound for branch-and-bound search.
	 */
	public static ReplicatedInteger createBoundVariable
		(int initialBound)
		{
		return createBoundVariable
			(initialBound, Comm.world(), Integer.MAX_VALUE);
		}

	/**
	 * Create a new shared, replicated <I>bound</I> variable.
	 *
	 * @param  initialBound    Initial bound for branch-and-bound search.
	 * @param  comm            Communicator used by the <I>bound</I> variable.
	 * @param  tag             Message tag used by the <I>bound</I> variable.
	...
	public static ReplicatedInteger createBoundVariable
		(int initialBound,
		 ...
		 int tag)
		{
		return new ReplicatedInteger
			(IntegerOp.MINIMUM, initialBound, tag, comm);
		}
...
// Exported constructors.
	 ...
	 * @param  bound    Shared, replicated <I>bound</I> variable.
	 * @param  results  Object in which to store the results.
	 */
	public MaximumParsimonyBnbHyb
...
// Exported operations.
	 ...
	 * <P>
	 * The results are stored in the {@linkplain MaximumParsimonyResults} object
	 * specified to the constructor. The <TT>findTrees()</TT> method will only
	 * find trees whose parsimony scores are less than or equal to the value of
	 * the <TT>bound</TT> variable specified to the constructor or the best
	 * bound found thereafter, whichever is smaller.
	 *
	 * @param  startLevel
	...
	public void findTrees
		...
		while (! done)
			...
			if (level == N)
				...
				// score.
				int updatedScore = bound.reduce (tentativeScore);
